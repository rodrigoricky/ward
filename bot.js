  ...
  * ————————
  * Archion Reborn v1.2.0
  * 
  * Lead Developer @ pineapple
  * Lead Developer @ eewee
  * 
  * ————————
  *  Bot Structure:
  *  
...
const { token, id, testguild }   		    = require("./config.js");
...
(async () => {
  try {
    ...
    await rest.put(
      ...
      Routes.applicationGuildCommands(id, testguild),
...
// ██████ | ███████████████████████████████████████████████████████████████████
	...
	const triggerFolders = fs.readdirSync("./messages/triggers/");

	// —— Loop through all files and store commands in commands collection.

	for (const folder of triggerFolders) {
		const triggerFiles = fs
			.readdirSync(`./messages/triggers/${folder}`)
			.filter((file) => file.endsWith(".js"));
		for (const file of triggerFiles) {
			const trigger = require(`./messages/triggers/${folder}/${file}`);
			client.triggers.set(trigger.name, trigger);
		}
	}


// —— Login into the MongoDB and Discord Application.
